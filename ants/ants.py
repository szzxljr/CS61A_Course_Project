...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        _range = 0
        while cur_place is not hive and _range <= self.max_range:
            enbees = cur_place.bees
            if enbees:
                return random_or_none(enbees)
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            for bees in self.place.bees[:]:
                Ant.reduce_armor(bees, self.damage)
            self.place.remove_insect(self)
        # END Problem 4
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting == 0:
            ...
            for bee in bees:
                self.eat_bee(bee)
                self.digesting = self.time_to_disgest
        else:
            self.digesting -= 1
        # END Problem 10
    ...
    def action(self, colony):
        ...
        if self.is_queen:
            ...
            while exit is not None:
                ant = exit.ant
                if ant is not None:
                    if ant not in self.ant_list:
                        ant.damage *= 2
                        self.ant_list.append(ant)
                    if ant.container and ant.ant is not None:
                        if ant.ant not in self.ant_list:
                            ant.ant.damage *= 2
                            self.ant_list.append(ant.ant)          
                exit = exit.exit
        else:
