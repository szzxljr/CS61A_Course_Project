...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return proce.eval_call(rest, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Argument values {0} do not match parameters numbers {1}".format(len(formals), len(vals)))
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        apl = self.apply(operands, env)
        args = complete_eval(apl)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if not scheme_symbolp(binding.first):
            raise SchemeError('Invalid symbol {0}.'.format(binding.first))
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        target = target.first
        body = expressions.second
        env.define(target, do_macro_form(Pair(formals, body), env))
        return target
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            proce = scheme_eval(first, env)
            check_procedure(proce)
            result = proce.eval_call(rest, env)
            #print(complete_eval(result))
            # END PROBLEM 20
    #print(result)
    return result
